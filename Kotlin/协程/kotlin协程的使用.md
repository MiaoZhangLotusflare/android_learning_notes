...
<mark>先写在这里，后面看放哪里合适</mark>

withTimeout

它是协程超时控制：给一段挂起代码设置最大执行时间，超时就取消这段协程并抛异常。

用途：防止网络请求、IO 等待状态无限卡住

使用：

```kotlin
val result = withTimeout(3_000) {
    api.fetchData()   // 超过 3 秒还没返回就超时
}
```

如果超时不想抛异常，可以用 withTimeoutOrNull：
...
withTimeout 时协作式取消，被包裹的挂起函数需要可取消才会及时停下来。

它只限制它包裹的那段协程代码，不是全局超时。
...
* withContext() 切换上下文是也会检查取消

* withTimeout() / withTimeoutOrNull()

* await() 系列（如 Deferred.await()）

* Flow.collect{...} 、first()、single() 等大多数 Flow 挂起操作
...
<mark>Mutext.lock()的功能</mark>



网络/库里通常可取消（取决于实现）：

* Retrofit suspend API（一般可取消，会联动取消底层 Call）

* Ktor client.get/post 等 suspend 请求（通常可取消）

* Room 的 suspend DAO 查询（通常可取消）



常见不一定可取消或取消不及时的：

* 用 Thread.sleep()、阻塞式 IO 包在 suspend 里（不是自动可取消）

* 纯 CPU 死循环不检查取消（例如 while(true){...}）

* withContext(NonCancellable){...} 里的代码（刻意不可取消）

* 第三方库的 suspend 只是语法包装，但内部没响应取消



怎么让自己的挂起逻辑可取消：

* 在循环中检查：coroutineContext.ensureActive() 或 isActive

* 用可取消挂起点（delay/yield）替代长时间阻塞

* 把阻塞 IO 放到 Dispatchers.IO，并优先用支持取消的 API

* 必要时捕获 CancellationException 后要继续抛出，并吞掉
